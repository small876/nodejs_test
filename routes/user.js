...
router.post('/purchase',verifyJWT, async(req, res)=>{
    ...
    try{
        ...
        let payload = jwt.verify(token, "secretKey",jwtSignOptions)
...
router.post('/register', async(req, res) => {
    try{    
        ...
        const token = jwt.sign({ user : user.account }, "secretKey", { expiresIn: '3000s' },jwtSignOptions)
...
router.post('/isExist', async(req, res) => {
    try{    
        ...
        if(IsAccountExist){
            ...
            res.status(200).json({message:'this account is useful'})
        }
    } catch(error) {
        res.status(500)
    }
